...
**그냥 실행해보고 싶다면 itch.io에서 받으세요.**

### ▶ [pianocchio.itch.io/solarsystem](https://pianocchio.itch.io/solarsystem)

빌드된 실행 파일과 텍스처가 모두 들어 있는 zip입니다. 압축을 풀고 `Ephemeris.exe`를
실행하면 끝입니다 — Vulkan SDK도, CMake도, 별도 텍스처 내려받기도 필요 없습니다.

압축을 풀 때 약 1.6 GB, 푸는 도중에는 원본 zip까지 3 GB 남짓 필요합니다.
**폴더 구조를 그대로 유지하세요** — 실행 파일이 `textures/`와 `shaders/`를 상대 경로로
찾습니다.

아래 [요구 사양](#요구-사양)은 이쪽으로 받아도 똑같이 적용됩니다. 특히 **VRAM 3 GB**와
**`maxImageDimension2D ≥ 16384`** 를 확인하세요.

소스를 보거나 직접 빌드하려면 [소스에서 빌드](#소스에서-빌드)로 가세요.
...
| GPU | Vulkan 1.2 지원 | — |
| 저장 공간 | itch.io **3 GB** / 소스 **4 GB** | — |
| RAM | 8 GB | 16 GB |

저장 공간이 두 가지인 이유: itch.io zip은 설치 후 1.6 GB지만 푸는 도중 원본 zip이
함께 있어야 합니다. 소스에서 빌드하면 클론 1.7 GB + Release 텍스처 0.85 GB + 빌드
산출물과 의존성 1 GB 남짓입니다. 텍스처가 이미 압축된 `.dds`라 git이 더 줄이지
못합니다. 히스토리가 필요 없다면 `git clone --depth 1`로 더 아낄 수 있습니다.
...
> 실행만 하실 거라면 [itch.io](https://pianocchio.itch.io/solarsystem)에서 받는 편이
> 훨씬 간단합니다. 이 절은 소스를 고치거나 직접 빌드하려는 경우입니다.
