...
public:
    ...
    // 프레임 경계. 이번 프레임의 CPU 델타를 누적한다(프레임 수는 세지 않는다).
    ...
    // collect() 다음에 호출한다. 프레임 카운터를 증가시킨다.
    void nextFrame();
...
private:
    ...
    bool   devTools_    = false;  // SOLAR_PROFILE=1 인가 (계측 UI와 콘솔 출력의 관문)
    bool   profiling_   = false; // 타임스탬프 기록 여부 (= devTools이고 장치가 지원하는가)
    int    frameCount_  = 0;
    double accumCpuRecord_ = 0.0, accumCpuFrame_ = 0.0, lastFrameStart_ = 0.0;
    double tsAccumGpu_[10]  = {};
    double tsSmoothGpu_[10] = {};  // 오버레이용. 지수 평활이라 숫자가 떨리지 않는다.
    VkDrawIndexedIndirectCommand lastDrawCmds_[12] = {}; // 직전 프레임의 LOD 바구니 스냅샷
};
